...
if (typeof ARIA2=="undefined"||!ARIA2) var ARIA2=(function(){
  ...
  return {
    init: function(path, onready) {
      var connect_msg_id = main_alert("alert-info", "正在连接到下载器 ...");
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "网络协议错误，请尝试刷新页面");
          ws_callback = {};
        };
        ...
        main_alert("alert-error", "未知的网络协议");
      };
    },
    ...
    restart_task: function(gids) {
      ...
      if (uris.length == 0) {
        main_alert("alert-error", "没有找到相关文件！ (BT 种子不支持断点续传)", 2000);
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          if (select_lock) return;
          if (!result.result) {
            main_alert("alert-error", "<strong>错误: </strong>RPC 服务器返回了错误的结果", 5000);
          }
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已清除全部已完成、已删除、失败的任务", 2000);
        }
      );
    },
    ...
    global_stat: function() {
      ARIA2.request("getGlobalStat", [],
        function(result) {
          ...
          $("#download-speed").empty().append(YAAW.tpl.view.format_size_0()(result.downloadSpeed)+"/s");
          $("#upload-speed").empty().append(YAAW.tpl.view.format_size_0()(result.uploadSpeed)+"/s");
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "参数变更成功", 2000);
          }
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表已经发生改变，请点击“刷新”按钮来更新列表");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
